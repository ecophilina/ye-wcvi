...
#       family = nbinom2(link = "log")
#     )
# saveRDS(m5a, file = "models/ye-m5-hook-count-400kn-AR1-only.rds")
m5a  <-  readRDS(file = "models/ye-m5-hook-count-400kn-AR1-only.rds")
m5a
tidy(m5a, "ran_pars",conf.int = TRUE)
get_diag(m5a)
```
...
# saveRDS(sp_cda, "models/ye-m5-spatial-predictions-cda.rds")
sp_cda <- readRDS("models/ye-m5-spatial-predictions-cda.rds")
...
# saveRDS(sp_noncda, "models/ye-m5-spatial-predictions-noncda.rds")
sp_noncda <- readRDS("models/ye-m5-spatial-predictions-noncda.rds")
...
nrow(cda_grid)/(nrow(cda_grid)+ nrow(noncda_grid))

(i_cda_sp$est/nrow(cda_grid)) 
(i_noncda_sp$est/nrow(noncda_grid)) 
```
...
p_3cd <- readRDS("models/ye-m5-AR1-predictions-3CD.rds")
sp_cda <- readRDS("models/ye-m5-spatial-predictions-cda.rds")
sp_noncda <- readRDS("models/ye-m5-spatial-predictions-noncda.rds")
g3cd <- ggplot(data = focal_area) +
   geom_tile(data = filter(sp_cda$data, year == 2020),
   aes(longitude, latitude, fill = exp(est)), width = 0.06, height = 0.025) +
   geom_tile(data = filter(sp_noncda$data, year == 2020),
    aes(longitude, latitude, fill = exp(est)), width = 0.06, height = 0.025) +
  # geom_tile(data = filter(p_3cd$data, year == 2020),
  #   aes(longitude, latitude, fill = exp(est)), width = 0.06, height = 0.025) +
  scale_fill_viridis_c(
    trans = ggsidekick::fourth_root_power_trans(),
    limits=c(0, 25), na.value = "yellow",
    option = "D") +
  labs(
    fill = "Estimated\ncount\nin 3CD",
    size = "Observed\ncount"
...
ggsave("ye-m5-spatial-2020-predictions-3cd.png", width = 6, height = 6)
```
