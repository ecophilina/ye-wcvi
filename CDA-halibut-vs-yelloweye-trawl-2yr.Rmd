...
# Check correlations in survey catch
...
d_ye_plotsN <- readRDS("data-generated/yelloweye-model-data-hbll-weights.rds") %>%
  filter(latitude > 50.01 & latitude < 51.4) %>% filter(year %in% c(#2017,
    2018,2019,2020)) %>% 
  mutate(
    # back transform densities into catch per 100 hooks
...
d_ye_plotsS <- readRDS("data-generated/yelloweye-model-data-hbll-weights.rds") %>%
  filter(latitude < 50.01)  %>% filter(year %in% c(#2017,
    2018,2019,2020)) %>% 
  mutate(
    # back transform densities into catch per 100 hooks
    catch_equiv = round(density * (100 * 0.0024384 * 0.009144 * 10000) / yemeanweight), 
...
p1 <- ggplot(catch_compN, aes(log(catch_equiv_hal+10), log(catch_equiv_ye+10))) + 
...
p3 <-ggplot(catch_compS, aes(log(catch_equiv_hal+10), log(catch_equiv_ye+10))) + 
...
if (!file.exists(f)) {
  ...
  i_hal_Sb <- get_index(p_hal_Sb, bias_correct = F) # bias_correction not working for full grid
  saveRDS(i_hal_Sb, f)
}
```
...
i_hal <- readRDS("data-generated/filled-keepable-halibut-est-rock-mud-index-hbll-S-sim-500.rds")[[1]]#[[1]]
...
ggplot(.dat, aes(year, est/1000, fill = Area, colour = Area)) + # convert kg to tonnes
  geom_line(size = 1.25) +
  geom_ribbon(aes(ymin = lwr/1000, ymax = upr/1000), alpha = 0.2, colour = NA) +
  xlab("Year") +
  ylab("Biomass index (landable tonnes)") +
  coord_cartesian(expand = FALSE, ylim = c(0, max(.dat$upr)/1000)) +
...
.dat$Area <- ordered(.dat$Area, levels = c("CDA", "non-CDA 3CD5A S of 50º", "3CD5A N of 50º", "5A", "Full combined", "HBLL grid only"))
ggplot(.dat, aes(year, est/1000, fill = Area, colour = Area)) +
  geom_line(size = 1.25) +
  geom_ribbon(aes(ymin = lwr/1000, ymax = upr/1000), alpha = 0.2, colour = NA) +
  # geom_line(data = .dat1, linetype = 2) +
  xlab("Year") +
...
ggsave("figs/filled-keepable-est-rock-mud-estimated-biomass-densities.png", width = 6, height = 5.5, dpi = 400)
```
...
ggplot(.dat2, aes(year, ye_per_hal, fill = Area, colour = Area)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
ggsave("figs/filled-keepable-ye-to-hal-index-est-rock-mud.png", width = 7, height = 4, dpi = 400)
...
# lowest years catch rate in kg per fish caught 
.datL <- left_join(.i_ye, .i_hal) %>% mutate(
    ye_per_hal = (yelloweye) / (halibut*0.62), 
    hal_per_ye = (halibut*0.62) / (yelloweye)
  )  %>% 
  group_by(Area, year) %>%
  summarise(
    lwr = quantile(ye_per_hal, 0.025),
...
.datL <- .datL %>% filter(Area %in% c(
  # "non-CDA 3CD5A",
  "non-CDA 3CD5A S of 50º", "5A3CD N of 50º",
  "CDA"))

# highest years catch rate in kg per fish caught 
.datH <- left_join(.i_ye, .i_hal) %>% mutate(
    ye_per_hal = (yelloweye) / (halibut*1.26), # convert to adam's estimates ye = 1.04
    hal_per_ye = (halibut*1.26) / (yelloweye)
  )  %>% 
  group_by(Area, year) %>%
  summarise(
    lwr = quantile(ye_per_hal, 0.025),
...
.datH <- .datH %>% filter(Area %in% c(
  # "non-CDA 3CD5A",
  "non-CDA 3CD5A S of 50º", "5A3CD N of 50º",
  "CDA"))

ggplot(.datH, aes(year, ye_per_hal, fill = Area, colour = Area)) +
  geom_line(aes(linetype = "solid")) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.4, colour = NA) +
  geom_line(data=.datL, aes(year, ye_per_hal, fill = Area, colour = Area, linetype = "dashed"
  )) +
  geom_ribbon(data=.datL, aes(ymin = lwr, ymax = upr), alpha = 0.2, colour = NA) +
  xlab("Year") +
  ylab("Estimated yelloweye to halibut biomass ratio") +
  coord_cartesian(expand = T, ylim = c(0.03, max(.datL$upr))) +
  # scale_x_continuous(breaks = seq(1950, 2050, 2)) +
  scale_linetype_manual(name = "Halibut", values= c("dashed", "solid"),
    label = c("lowest landings",  "most landings")) +
  scale_x_continuous(
    labels=c("2007-2008", "2009-2010", "2011-2012", "2013-2014", "2015-2016","2017-2018","2019-2020"), 
    breaks = seq(2008, 2020, 2)) + 
  theme(plot.margin = margin(11 / 2, r = 11, 11 / 2, 11 / 2)) +
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
ggsave("figs/filled-keepable-ye-to-hal-sizes-index-est-rock-mud.png", width = 7, height = 4, dpi = 400)
# 
...
#   scale_fill_brewer(palette = "Set1") +
#   scale_colour_brewer(palette = "Set1")
# ggsave("figs/filled-keepable-halx-to-ye-index-est-rock-mud.png", width = 7, height = 4, dpi = 400)
```
...
bjs <- tidy(m_halibut)
# bjs[bjs$term == "as.factor(survey)TRAWL",2]
# exp(bjs[bjs$term == "as.factor(survey)TRAWL",2])
...
d_hal_plots <- readRDS("data-generated/halibut-model-data-keepable-weight.rds") %>%
  ...
  filter(year %in% c(#2017,
    2018,2019,2020)) %>%  
  mutate(
    density_adj = ifelse(survey == "TRAWL", density/exp(bjs[bjs$term == "as.factor(survey)TRAWL",2]), density),
    # back transform densities into catch per 100 hooks
    catch_equiv = round(density * (100 * 0.0024384 * 0.009144 * 10000) / halmeanweight), 
    max_count = round((max(catch_count, na.rm = T)/mean(hook_count, na.rm = T))*100),
    catch_equiv2 = ifelse(catch_equiv < max_count, catch_equiv, max_count)
...
g <- map_predictions(
  ...
  size_aes = density_adj
  # size_lab = "Adjusted catch\ncounts", #2018-2020\observed catch\nper 100 hook\nequivalent
  # size_aes = catch_equiv 
)+theme(legend.spacing.y = unit(0.1, "cm"))
g
dir.create("figs", showWarnings = FALSE)
ggsave("figs/filled-keepable-halibut-2020-predictions-est-rock-mud.png", width = 6, height = 5, dpi = 400)
```
...
g
dir.create("figs", showWarnings = FALSE)
ggsave("figs/filled-keepable-yelloweye-2020-predictions-est-rock-mud.png", width = 6, height = 5, dpi = 400)
```
...
p_ye <- readRDS("data-generated/filled-keepable-yelloweye-est-rock-mud-predictions-all-S.rds")[[1]]
p_hal <- readRDS("data-generated/filled-keepable-halibut-est-rock-mud-predictions-all-S.rds")[[1]]
pyd <- p_ye %>% rename(ye_est = est) %>% distinct()
phd <- p_hal %>%
  rename(hal_est = est) %>%
  select(hal_est, X, Y, year)%>% distinct()
...
dir.create("figs", showWarnings = FALSE)
ggsave("figs/filled-keepable-ye-to-halibut-2020-w-hal-catch-est-rock-mud.png", 
  width = 6, height = 5, dpi = 200)
# ggsave("figs/filled-keepable-ye-to-halibut-2020-w-hal-catch-S.pdf", width = 5.5, height = 6, dpi = 400)
```
...
dir.create("figs", showWarnings = FALSE)
ggsave("figs/filled-keepable-halibut-to-ye-2020-w-hal-catch-est-rock-mud.png", 
  width = 6, height = 5, dpi = 200)
