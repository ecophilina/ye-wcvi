...
```{r}
f <- "grids/norca_grid_w_substrate_expanded.rds"
...
full_s_grid <- full_s_grid %>% filter(!(latitude < 50.85 & latitude > 50.75 & longitude > -128.94 & longitude < -128.7))
...
ggsave("figs/region-colour-map-3x3-expanded.png", width = 5, height = 3, dpi = 300)
}
```
...
g <- ggplot(focal_area_proj) +
  ...
  geom_tile(data = full_s_grid, aes(X * 100000, Y * 100000),
...
ggsave("figs/map-stitched-grid-overlap-expanded2.png", width = 5.5, height = 5.5, dpi = 400)
}
```
...
full_s_grid3 <- filter(full_s_grid, latitude < 50.85 & latitude > 50.75 & longitude > -128.94 & longitude < -128.7)

full_s_grid3$area/1000000

ggplot() + geom_point(data = filter(full_s_grid, 
  ...
  geom_point(data = filter(full_s_grid, 
    latitude < 50.85 & latitude > 50.75 & longitude > -128.94 & longitude < -128.7
    ), aes(longitude, latitude))
```
