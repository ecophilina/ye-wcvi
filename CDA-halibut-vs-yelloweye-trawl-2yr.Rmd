...
ratio_noncda <- ratio_noncda[order(ratio_noncda$halibut, decreasing = T), ] %>% group_by(year) %>% 
  mutate(ordered = 1:n(),
    hal_cumsum = cumsum(halibut), 
    ye_cumsum = cumsum(yelloweye)) %>% ungroup()
...
ggplot() +
  ...
  coord_cartesian(expand = F, xlim = c(0, 300), ylim = c(0, target_catch )) +
  ...
  ylab("Fish biomass (kg per 100 hooks per cell") +
  xlab("Number of cells fished (in order of decreasing Halibut abundance)") + 
  theme(legend.position = c(0.65, 0.1))

ggsave("cummulative-catch-rates.png", width = 7, height = 9)
```
...
```{r}
ggplot(target_noncda,  aes(year, ye_total)) + geom_line(colour = "darkblue") + 
  geom_line(data = target_cda,  aes(year, ye_total), colour = "red") +
  scale_x_continuous(
    labels=c("2007-2008", "2009-2010", "2011-2012", "2013-2014", "2015-2016","2017-2018","2019-2020"), 
    breaks = seq(2008, 2020, 2)) + 
  xlab("Year") +
  ylab("Expected catch of YE if 1500 kg of halibut targeted")
