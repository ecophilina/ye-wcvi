...
ratio_cda <- ratio_cda[order(ratio_cda$halibut, decreasing = T), ] %>% group_by(year) %>% mutate(ordered = 1:n()) %>% ungroup()
ratio_noncda <- ratio_noncda[order(ratio_noncda$halibut, decreasing = T), ] %>% group_by(year) %>% mutate(ordered = 1:n()) %>% ungroup()
...
ggplot(
  ...
  geom_line(data = ratio_noncda, 
    aes(x = ordered, y = cumsum((halibut)), lty = "dotted", colour = "black"), size = 1,
    inherit.aes = FALSE) +
  geom_line(data = ratio_cda, 
    aes(x = ordered, y = cumsum((halibut)), lty = "dotted", colour = "red"), size = 1, 
    inherit.aes = FALSE) +
  geom_line(data = ratio_noncda, 
    aes(x = ordered, y = cumsum((yelloweye)), lty = "solid", colour = "black"), size = 1,
    inherit.aes = FALSE) +
  geom_line(data = ratio_cda, 
    aes(x = ordered, y = cumsum((yelloweye)), lty = "solid", colour = "red"), size = 1, 
    inherit.aes = FALSE) +
  coord_cartesian(expand = F, xlim = c(0, 300), ylim = c(0, 4500)) +
...
ggsave("cummulative-catch-rates.png", width = 5.5, height = 9)
```
