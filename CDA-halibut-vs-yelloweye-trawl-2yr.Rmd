...
full_s_grid$region <- ordered(full_s_grid$region, 
  ...
  labels = c("CDA", "other", "3CD5A N of 50º", "non-CDA 3CD5A S of 50º"))
...
g <- g + leg + patchwork::plot_layout(widths = c(1, 0.5))
ggsave("figs/region-colour-map-3x3.png", width = 5, height = 3, dpi = 100)
}
```
...
ggplot(filter(.dat, Area %in% c("CDA", "non-CDA 3CD5A")), aes(year, est)) +
  geom_line(aes(colour = Area), size = 1.5) +
  geom_ribbon(aes(
    ymin = lwr,
    ymax = upr, fill = Area
