...
# p1p <-p%>% filter(density >0)
# 
# p$resids <- r_hal_tw
# 
# off <- p[which(r_hal_tw > 2.4|r_hal_tw < -2.4), ]
# ggplot(p, aes(density, exp(est), shape = survey)) + geom_point(size = 1) +
#   geom_abline(a=0, b=1) +
#   geom_point(data = off, colour = "red")
# 
# p2 <- predict(m_halibut_bin)
# p2p <-p2%>% filter(present ==1)
# 
# ggplot(p2, aes(density, exp(est), shape = survey)) + geom_point(size = 1) +
#   geom_abline(a=0, b=1)
# 
# p3 <- predict(m_halibut_pos) 
# p3p <-p3%>% filter(present ==1)
# ggplot(p3, aes(density, exp(est), shape = survey)) + geom_point(size = 1) +
#   geom_abline(a=0, b=1)
# 
# delta_est <- log(plogis(p2p$est) * exp(p3p$est))
# 
# plot(delta_est,p1p$est)

```


```{r echo = F}
m_halibut_tweedie <- readRDS(file = "models/halibut-hybrid-keepable-model-rocky-muddy-400kn-tweedie.rds")
m_halibut_tweedie
# # round(AIC(m_halibut))
...
```
```{r}
m_halibut_bin <- readRDS(file = "models/halibut-hybrid-keepable-model-rocky-muddy-400kn-binomial.rds")
m_halibut_bin
m_halibut_pos <- readRDS(file = "models/halibut-hybrid-keepable-model-rocky-muddy-400kn-gamma.rds")
m_halibut_pos
```
...
```{r  echo=F}
# f <- "data-generated/filled-keepable-halibut-delta-est-rock-mud-predictions-5A3CD-outside-cda.rds"
# 
# if (!file.exists(f)) {
#   p_hal_noncda_bin <- predict(m_halibut_bin, newdata = noncda_grid, return_tmb_object = TRUE)
#   p_hal_noncda_pos <- predict(m_halibut_pos, newdata = noncda_grid, return_tmb_object = TRUE)
# 
#   p_hal_noncda <-  p_hal_noncda_bin
#   p_hal_noncda$data$est_bin <- p_hal_noncda$data$est
#   p_hal_noncda$data$est_pos <- p_hal_noncda_pos$data$est
#   p_hal_noncda$data$est <- log(plogis(p_hal_noncda$data$est_bin) * exp(p_hal_noncda$data$est_pos))
# 
#   saveRDS(p_hal_noncda, f)
# }
...
```{r}
# f <- "data-generated/filled-keepable-halibut-delta-est-rock-mud-predictions-cda.rds"
# if (!file.exists(f)) {
#   
#   p_hal_cda_bin <- predict(m_halibut_bin, newdata = cda_grid, return_tmb_object = TRUE)
#   p_hal_cda_pos <- predict(m_halibut_pos, newdata = cda_grid, return_tmb_object = TRUE) 
# 
#   p_hal_cda <-  p_hal_cda_bin
#   p_hal_cda$data$est_bin <- p_hal_cda$data$est
#   p_hal_cda$data$est_pos <- p_hal_cda_pos$data$est
#   p_hal_cda$data$est <- log(plogis(p_hal_cda$data$est_bin) * exp(p_hal_cda$data$est_pos))
#   saveRDS(p_hal_cda, f)
# }
```
...
ggsave("figs/filled-keepable-halibut-delta-2020-predictions-est-rock-mud.png", width = 6, height = 5, dpi = 400)
```
...
dir.create("figs", showWarnings = FALSE)
ggsave("figs/filled-keepable-halibut-delta-to-ye-2020-w-hal-catch-est-rock-mud.png", 
  width = 5.5, height = 4.5, dpi = 400
  # width = 6, height = 5, dpi = 200
  )
# ggsave("figs/filled-keepable-halibut-delta-to-ye-2020-w-hal-catch-S.pdf", width = 5.5, height = 6, dpi = 400)
```
...
```{r message=F, echo=F, warning=F}
p_hal_cda <- readRDS("data-generated/filled-keepable-halibut-delta-est-rock-mud-predictions-cda.rds")
p_ye_cda <- readRDS("data-generated/filled-keepable-yelloweye-est-rock-mud-predictions-cda.rds")

p_hal_noncda <- readRDS("data-generated/filled-keepable-halibut-delta-est-rock-mud-predictions-5A3CD-outside-cda.rds")
p_ye_noncda <- readRDS("data-generated/filled-keepable-yelloweye-est-rock-mud-predictions-5A3CD-outside-cda.rds")

# p_hal_5A <- readRDS("data-generated/filled-keepable-halibut-delta-est-rock-mud-predictions-5A.rds")
