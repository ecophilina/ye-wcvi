...
ggsave("figs/hbll-catch-correlations.png", width = 3.5, height = 5.5, dpi = 400)
...
```{r}
f <- paste0("figs/region-colour-map-3x3.png")
if (!file.exists(f)) {
full_s_grid$region <- ordered(full_s_grid$region, 
  levels = c("CDA", "other", "3CD5A N", "non-CDA 3CD5A S"), 
  labels = c("CDA", "non-CDA 3CD5A", "3CD5A N of 50º", "non-CDA 3CD5A S of 50º"))

g1 <- ggplot(full_s_grid, aes(X,Y, colour = region)) + geom_point() +
...
g <- map_predictions(
  pred_data = full_s_grid,
  fill_aes = region,
  fill_lab = "Region",
  map_lat_limits = c(48.6, 51.25),
  map_lon_limits = c(min_map_lon, max_map_lon)
)+
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
g <- g + leg + patchwork::plot_layout(widths = c(1, 0.55))
ggsave("figs/region-colour-map-3x3.png", width = 5, height = 3, dpi = 300)
}
```
...
f <- paste0("models/halibut-hybrid-keepable-model-rocky-muddy-400kn-tweedie.rds")
if (!file.exists(f)) {
...
f <- paste0("models/halibut-hybrid-keepable-model-rocky-muddy-400kn-binomial.rds")
if (!file.exists(f)) {
  d_hal$present <- ifelse(d_hal$density > 1, 1, 0)
...
f <- paste0("models/halibut-hybrid-keepable-model-rocky-muddy-400kn-gamma.rds")
if (!file.exists(f)) {
...
m_halibut_tweedie <- readRDS(paste0("models/halibut-hybrid-keepable-model-rocky-muddy-400kn-tweedie.rds"))
m_halibut_bin <- readRDS(paste0("models/halibut-hybrid-keepable-model-rocky-muddy-400kn-binomial.rds"))
m_halibut_pos <- readRDS(paste0("models/halibut-hybrid-keepable-model-rocky-muddy-400kn-gamma.rds"))

r_hal_tw <- residuals(m_halibut_tweedie)
r_hal_bin <- residuals(m_halibut_bin)
r_hal_pos <- residuals(m_halibut_pos)
png("figs/halibut-qq.png", width = 10, height = 3.7, units = "in", res = 200)
...
# 
# d_hal$off <- ifelse(r_hal0 > 2.5, TRUE, FALSE)
# ggplot(d_hal, aes(X, Y, colour = density, shape = survey)) + geom_point(size = 1) +
#   facet_wrap(~year) +
#   scale_color_viridis_c(trans = "sqrt") +
#   coord_equal() +
...
r_ye0 <- residuals(m_ye)
png("figs/ye-qq.png", width = 5, height = 4.7, units = "in", res = 200)
qqnorm(r_ye0, main = "Tweedie");qqline(r_ye0)
dev.off()

# d_ye$present <- ifelse(d_ye$density > 1, 1, 0)
# m_ye1 <- sdmTMB(
#     formula = present ~ 0 + as.factor(year) + as.factor(survey) + 
#       s(rocky, k = 3) +
#       s(muddy, k = 3) +
...
## duplicates subregions saved in chunk hal-sim-subregions
...
if (!file.exists(ss)) {
  i_hal_noncda2 <- get_all_sims(m_halibut, newdata = noncda_grid, split_by_region = F)
...
# have not saved simulated index for this whole area...
```
...
if (!file.exists(ss)) {
  i_hal_S2b <- get_all_sims(fit_obj_bin = m_halibut_bin, fit_obj_pos = m_halibut_pos, newdata = hbll_s_grid, split_by_region = F)
...
## duplicates subregions saved in chunk ye-sim-subregions
...
saveRDS(i_hal, here("report-data/filled-keepable-halibut-est-rock-mud-index-all-S-sim-500.rds"))
i_hal_cda <- readRDS(here("data-generated/filled-keepable-halibut-est-rock-mud-index-cda-sim-500.rds"))[[1]]
saveRDS(i_hal_cda, here("report-data/filled-keepable-halibut-est-rock-mud-index-cda-sim-500.rds"))
i_hal_noncda <- readRDS(here("data-generated/filled-keepable-halibut-est-rock-mud-index-5A3CD-outside-cda-sim-500.rds"))[[1]]
saveRDS(i_hal_noncda, here("report-data/filled-keepable-halibut-est-rock-mud-index-5A3CD-outside-cda-sim-500.rds"))
i_hal_noncdaN <- readRDS(here("data-generated/filled-keepable-halibut-est-rock-mud-index-5A3CDN-outside-cda-sim-500.rds"))[[1]]
saveRDS(i_hal_noncdaN, here("report-data/filled-keepable-halibut-est-rock-mud-index-5A3CDN-outside-cda-sim-500.rds"))
i_hal_noncdaS <- readRDS(here("data-generated/filled-keepable-halibut-est-rock-mud-index-5A3CDS-outside-cda-sim-500.rds"))[[1]]
saveRDS(i_hal_noncdaS, here("report-data/filled-keepable-halibut-est-rock-mud-index-5A3CDS-outside-cda-sim-500.rds"))
```
...
g_i_hal <- ggplot(.datH, aes(year, est/1000, fill = Area, colour = Area)) + # convert kg to tonnes
  ...
  coord_cartesian(expand = FALSE, ylim = c(0, max(.datH$upr)/1000)) +
...
g_i_ye <- g_i_ye + 
  coord_cartesian(ylim = c(0, max(.datH$est)/1000)) +
  theme(
...
ggplot(.datH, aes(year, est)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
ggsave("figs/filled-keepable-halibut-est-rock-mud-average-index-NS.png", width = 7, height = 4, dpi = 400)
...
ggsave("figs/filled-keepable-rock-mud-avg-by-area.png", width = 6, height = 7, dpi = 300)
...
ggplot(.dat2, aes(year, ye_per_hal, fill = Area, colour = Area)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
ggsave("figs/filled-keepable-ye-to-hal-index-est-rock-mud.png", width = 7, height = 3.4, dpi = 400)
...
g
dir.create("figs", showWarnings = FALSE)
ggsave("figs/filled-keepable-halibut-2020-predictions-est-rock-mud-closeup.png", width = 5.5, height = 4.5, dpi = 400)
```
