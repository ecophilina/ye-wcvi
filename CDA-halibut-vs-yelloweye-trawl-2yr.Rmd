...
# m_halibut <- readRDS(file = "models/halibut-hybrid-model-allyrs-400kn.rds")
...
# m_halibut <- readRDS(file = "models/halibut-hybrid-model-AR1-allyrs-400kn.rds")
m_halibut2
AIC(m_halibut2) 
# delta 5 AIC better, but probably not worth the added complexity?
```
...
```{r}
# m_ye <- readRDS(file = "models/yelloweye-hybrid-400kn-spatial-only-trawlinhbllyrs.rds")
# m_ye <- readRDS(file = "models/yelloweye-hybrid-AR1-allyrs-400kn.rds")
# m_ye <- readRDS(file = "models/yelloweye-hybrid-allyrs-400kn.rds")
# m_ye <- readRDS(file = "models/yelloweye-hybrid-trawlinhbllyrs-paired-AR1-400kn.rds")
...
## Compare indices between species and model types for full HBLL S grid only
...
.dat <- bind_rows(mutate(i_hal, Model = "HBLL + trawl", Species = "Halibut"),
  mutate(i_hal_hbll, Model = "HBLL", Species = "Halibut", 
    est = est * halmeanweight, lwr = lwr * halmeanweight, upr = upr * halmeanweight),
  mutate(i_ye, Model = "HBLL + trawl", Species = "Yelloweye"),
  mutate(i_ye_hbll, Model = "HBLL",  Species = "Yelloweye", 
    est = est * yemeanweight, lwr = lwr * yemeanweight, upr = upr * yemeanweight)
)
...
ggsave("figs/compare-model-indices.png", width = 4, height = 5, dpi = 400)
```

# Results

## Index of total halibut catch through time
...
.dat <- bind_rows(
  mutate(i_hal, Area = "Full combined grid"),
...
.dat$Area <- ordered(.dat$Area, levels = c("CDA", "non-CDA 3CD", "5A", "Full combined grid", "HBLL grid only"))
...
## Estimated halibut catch per 100 hooks through time
...
.dat <- bind_rows(
  mutate(i_ye, Area = "Full combined grid"),
...
## Plot estimated yelloweye catch per 100 hooks through time
...
.dat2 <- bind_rows(
  mutate(i_hal2, Area = "Full combined grid", num_cells = nrow(full_s_grid)),
...
.dat2$Area <- ordered(.dat2$Area, levels = c("CDA", "non-CDA 3CD", "5A", "Full combined grid"))
...
.dat2 <- bind_rows(
  mutate(i_ye2, Area = "Full combined grid", num_cells = nrow(full_s_grid)),
...
ggplot(i_hal_2020) + geom_histogram(aes(.value / num_cells), alpha = 0.75, fill = "black") +
  ...
  ggtitle("Estimated relative biomass densities (median and distribution)",
    subtitle = "Yelloweye Rockfish (coloured bars) vs. Pacific Halibut (grey bars)"
  )

ggsave("figs/hybrid-trawlinhbllyrs-paired-estimated-biomass-densities.png", width = 5, height = 6, dpi = 400)
```
...
hal_ratios <- i_hal_2020 %>%
  ...
  group_by(.iteration) %>%
  mutate(CDAto3CD = CDA / (CDA + `non-CDA 3CD`), CDAtoFull = CDA / `Full combined grid`)
...
ye_ratios <- i_ye_2020 %>%
  ...
  pivot_wider(names_from = Area, values_from = .value) %>%
  group_by(.iteration) %>%
  mutate(CDAto3CD = CDA / (CDA + `non-CDA 3CD`), CDAto3CD5A = CDA / (`Full combined grid`))
...
.i_ye$Area <- ordered(.i_ye$Area, levels = c("CDA", "non-CDA 3CD", "5A", "Full combined grid"))
...
.i_hal$Area <- ordered(.i_hal$Area, levels = c("CDA", "non-CDA 3CD", "5A", "Full combined grid"))
...
ggplot(.dat, aes(year, hal_per_ye, fill = Area, colour = Area)) +
  ...
  coord_cartesian(expand = FALSE, ylim = c(0, max(.dat$upr_hal) * 1.04)) +
  scale_x_continuous(breaks = seq(1950, 2050, 2)) +
  theme(plot.margin = margin(11 / 2, r = 11, 11 / 2, 11 / 2)) +
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
...
```{r message=F, echo=F}
p_hal2020 <- readRDS("data-generated/hybrid-halibut-trawlinhbllyrs-paired-predictions-all-S.rds")[[1]] %>% filter(year == 2020)
d_hal_plots <- readRDS("data-generated/halibut-hybrid-model-data.rds") %>%
  ...
  filter(latitude < 52.15507) %>% filter(year %in% c(2017,2018,2019,2020)) %>%  
  mutate(
