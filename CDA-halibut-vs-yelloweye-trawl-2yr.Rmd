...
.dat$Area <- ordered(.dat$Area, levels = c("CDA", "non-CDA 3CD5A", "5A", "Full combined", "HBLL grid only"))
...
```{r message=F, echo=F}
ggplot(i_hal_2020 %>% filter(Area %in% c("CDA", "non-CDA 3CD5A"))) + geom_histogram(aes(.value / num_cells, 
  fill = Area
  ), alpha = 0.5
  #, fill = "black"
  ) +
  geom_histogram(data = i_ye_2020 %>% filter(Area %in% c("CDA", "non-CDA 3CD5A")), aes(.value / num_cells, fill = Area), alpha = 0.75) +
  geom_vline(data = catch_per_cell%>% filter(Area %in% c("CDA", "non-CDA 3CD5A")), aes(xintercept = catch_med,
    colour = Area
    ), linetype = 2, alpha = 0.5
    #, colour = "black"
    ) +
  geom_vline(data = ye_catch_per_cell%>% filter(Area %in% c("CDA", "non-CDA 3CD5A")), aes(xintercept = catch_med, colour = Area), linetype = 2) +
