...
.dat2 <- bind_rows(
  ...
  mutate(i_hal_noncda2S, Area = "non-CDA 5A3CD S N of 50 degrees", num_cells = nrow(noncda_gridS)),
  mutate(i_hal_cda2, Area = "CDA", num_cells = nrow(cda_grid))
)
.dat2$Area <- ordered(.dat2$Area, levels = c("CDA", "non-CDA 5A3CD", "5A3CD N of 50 degrees", "non-CDA 5A3CD S N of 50 degrees", "5A", "Full combined"))
...
.dat2 <- bind_rows(
  ...
  mutate(i_ye_noncda2S, Area = "non-CDA 5A3CD S N of 50 degrees", num_cells = nrow(noncda_gridS)),
...
mean_hal_ratios <- hal_ratios %>%
  pivot_longer(cols = c(CDAto5A3CD, CDAtoFull), names_to = "area", values_to = "ratio") %>%
  group_by(area) %>%
  summarise(
...
ggplot(hal_ratios) +
  ...
  xlim(0, max(hal_ratios$CDAto5A3CD)) +
  xlab("Proportion of total catch inside CDA") + ylab("Number of simulations") +
  ggtitle("Pacific Halibut survey biomass in area 5A3CD (red) and full combined grid (grey)")
...
ye_ratios <- i_ye_2020 %>%
  ...
  mutate(CDAto5A3CD = CDA / (CDA + `non-CDA 5A3CD`), 
    ...
    prop5A3CDS = `non-CDA 5A3CD S N of 50 degrees`/`non-CDA 5A3CD`
    )
