...
ggplot(i_hal_2020) + geom_histogram(aes(.value / num_cells * halmeanweight), alpha = 0.75, fill = "black") +
  ...
  scale_fill_brewer(palette = "Set1") + scale_colour_brewer(palette = "Set1") + theme(legend.position = "none") +
  ggtitle("Estimated biomass of catch (mean and distribution)",
    subtitle = paste0("Yelloweye Rockfish (mean of ",yemeanweight,"kg per fish; coloured bars) vs. Pacific Halibut (mean of ",halmeanweight,"kg per fish; grey bars)")
  )
...
g <- map_predictions(
  ...
  obs_data = filter(d_hal, year == 2020),
  pred_min = 0, 
  pred_max = max(d_hal$catch_count, na.rm = T)/ mean(d_hal$hook_count, na.rm = T) * 100,
  fill_aes = hal_per_ye,
  fill_lab = "Predicted count\nof Halibut per Yelloweye",
  size_aes = (catch_count / hook_count) * 100,
  size_lab = "Halibut caught\nper 100 hooks"
)
g
...
dir.create("figs", showWarnings = FALSE)
ggsave("figs/halibut-per-yelloweye-2020-w-hal-catch-S.png", width = 5.5, height = 6, dpi = 400)
```
...
ggsave("figs/ye-to-halibut-2020-w-hal-catch-S.pdf", width = 5.5, height = 6, dpi = 400)
```
