...
saveRDS(p_hal_noncda, "data-generated/halibut-predictions-S-outside-cda.rds")
...
bound5Bnorth <- fortify(majorbound) %>% 
  filter(PID %in% c(6)) %>%  filter( #POS != 46 & # strange jog over HG
    X > 200 & X < 560 & Y> 5700 )%>% gfplot:::utm2ll(utm_zone = 9)
bound5Anorth <- fortify(majorbound) %>% 
  filter(PID %in% c(5)) %>%  filter(
    X > 200 & X < 560 & Y> 5650 )%>% gfplot:::utm2ll(utm_zone = 9)
# 3CD 
bound3Cnorth <- fortify(majorbound) %>% 
  filter(PID %in% c(3)) %>%  filter(X > 200 & X < 725 & Y > 5400)%>% gfplot:::utm2ll(utm_zone = 9)
bound3Dnorth <- fortify(majorbound) %>% 
  filter(PID %in% c(4)) %>%  filter(X > 200 & X < 600 & Y > 5550)%>% gfplot:::utm2ll(utm_zone = 9)
```
