...
sim_hal_cda <- readRDS("data-generated/halibut-index-cda-sim-200.rds")[[1]]
sim_hal_noncda <- readRDS("data-generated/halibut-index-3CD-outside-cda-sim-200.rds")[[1]]
...
ggplot(.dat, aes(year, est)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
ggsave("figs/halibut-average-index-by-area.png", width = 7, height = 4, dpi = 400)
```
...
ggplot(.dat, aes(year, est, fill = Area, colour = Area)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
ggsave("figs/yelloweye-index-by-area.png", width = 7, height = 4, dpi = 400)
```
...
ggplot(.dat, aes(year, est)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  scale_colour_brewer(palette = "Set1")
ggsave("figs/yelloweye-average-index-by-area.png", width = 7, height = 4, dpi = 400)
```
...
round(i_hal_cda2020$est / nrow(cda_grid), 2)
```
...
round(i_hal_noncda2020$est / nrow(noncda_grid),2)
...
i_hal_cda2020 <- filter(i_hal_cda, year == 2020)
i_hal_noncda2020 <- filter(i_hal_noncda, year == 2020)
i_hal_5A <- filter(i_hal_5A, year == 2020)

print("Average estimated catch per 100 hooks outside CDA:")
round((i_hal_noncda2020$est + i_hal_5A$est)/(nrow(noncda_grid)+nrow(s_5A_grid)),2)

print("What proportion of estimated total catch is inside CDA in 2020?")
round(i_hal_cda2020$est / (i_hal_cda2020$est + i_hal_noncda2020$est + i_hal_5A$est), 2)
```
...
print("Average estimated catch per 100 hooks inside CDA:")
round(i_ye_cda2020$est / nrow(cda_grid), 2)
```
...
print("Average estimated catch per 100 hooks outside CDA:")
round(i_ye_noncda2020$est / nrow(noncda_grid), 2)

print("What proportion of estimated total catch is inside CDA?")
round(i_ye_cda2020$est / (i_ye_cda2020$est + i_ye_noncda2020$est), 2)
```
...
print("Average estimated catch per 100 hooks outside CDA:")
round((i_ye_noncda2020$est + i_ye_5A2020$est)/(nrow(noncda_grid)+nrow(s_5A_grid)),2)

print("What proportion of estimated total catch is inside CDA in 2020?")
round(i_ye_cda2020$est / (i_ye_cda2020$est + i_ye_noncda2020$est + i_ye_5A2020$est), 2)
```
...
.dat1 <- bind_rows(
  ...
  mutate(i_hal_noncda, Area = "non-CDA 3CD"),
  mutate(i_hal_cda, Area = "CDA")
) %>% rename(est_hal = est, lwr_hal = lwr, upr_hal = upr)
...
.dat2 <- bind_rows(
  ...
  mutate(i_ye_noncda, Area = "non-CDA 3CD"),
  mutate(i_ye_cda, Area = "CDA")
) %>% rename(est_ye = est, lwr_ye = lwr, upr_ye = upr)
.dat2$Area <- ordered(.dat2$Area, levels = c("CDA", "non-CDA 3CD", "5A", "HBLL South"))

.dat <- left_join(.dat1, .dat2, by= c("year", "Area")) %>% mutate(ye_per_hal = est_ye/est_hal, 
  lwr = lwr_ye/est_hal, upr = upr_ye/est_hal
  # lwr = est_ye/upr_hal, upr = est_ye/lwr_hal # halibut index is less uncertain
  # lwr = lwr_ye/upr_hal, upr = upr_ye/lwr_hal # if think using all the uncertainty amounts to ~99.7% CIs
...
# calculate mean weigths for all of HBLL S
