...
ss <- "data-generated/halibut-index-cda-sim-200.rds"
if (!file.exists(ss)) {
...
ss <- "data-generated/halibut-index-3CD-outside-cda-sim-200.rds"
if (!file.exists(ss)) {
...
ss <- "data-generated/halibut-index-5A-sim-200.rds"
if (!file.exists(ss)) {
...
ss <- "data-generated/halibut-index-all-S-sim-200.rds"
if (!file.exists(ss)) {
...
i_hal <- readRDS("data-generated/halibut-index-all-S-sim-200.rds")[[1]]
i_hal_cda <- readRDS("data-generated/halibut-index-cda-sim-200.rds")[[1]]
i_hal_noncda <- readRDS("data-generated/halibut-index-3CD-outside-cda-sim-200.rds")[[1]]
i_hal_5A <- readRDS("data-generated/halibut-index-5A-sim-200.rds")[[1]]
...
ss <- "data-generated/yelloweye-spatial-index-cda-sim-200.rds"
if (!file.exists(ss)) {
...
ss <- "data-generated/yelloweye-spatial-index-3CD-outside-cda-sim-200.rds"
if (!file.exists(ss)) {
...
ss <- "data-generated/yelloweye-spatial-index-5A-sim-200.rds"
if (!file.exists(ss)) {
...
ss <- "data-generated/yelloweye-spatial-index-all-S-sim-200.rds"
if (!file.exists(ss)) {
...
i_ye <- readRDS("data-generated/yelloweye-spatial-index-all-S-sim-200.rds")[[1]]
i_ye_cda <- readRDS("data-generated/yelloweye-spatial-index-cda-sim-200.rds")[[1]]
i_ye_noncda <- readRDS("data-generated/yelloweye-spatial-index-3CD-outside-cda-sim-200.rds")[[1]]
i_ye_5A <- readRDS("data-generated/yelloweye-spatial-index-5A-sim-200.rds")[[1]]
...
```{r}
i_hal_cda2 <- readRDS("data-generated/halibut-index-cda-sim-200.rds")[[2]]
i_hal_noncda2 <- readRDS("data-generated/halibut-index-3CD-outside-cda-sim-200.rds")[[2]]
i_hal_5A2 <- readRDS("data-generated/halibut-index-5A-sim-200.rds")[[2]]
i_hal2 <- readRDS("data-generated/halibut-index-all-S-sim-200.rds")[[2]]
...
catch_per_cell <- i_hal_2020 %>% group_by(Area) %>% summarise(
...
ggplot(i_hal_2020) + geom_histogram(aes(.value/num_cells, fill = Area)) + 
  geom_vline(data = catch_per_cell, aes(xintercept = catch), linetype = 2) +
...
ggsave("figs/halibut-mean-catch-sim.png", width = 3, height = 4, dpi = 400)
```

What proportion of estimated total catch is inside CDA in 2020?

```{r echo=F}
hal_ratios <- i_hal_2020 %>% select(-num_cells) %>% pivot_wider(names_from = Area, values_from = .value) %>% 
  group_by(.iteration) %>% mutate(CDAto3CD = CDA/(CDA + `non-CDA 3CD`), CDAto3CD5A = CDA/(CDA + `non-CDA 3CD` + `5A`))

mean_hal_ratios <- hal_ratios %>% 
  pivot_longer(cols = c(CDAto3CD, CDAto3CD5A), names_to = "area", values_to = "ratio") %>% group_by(area) %>% 
...
## Yelloweye Rockfish
```{r}
i_ye_cda2 <- readRDS("data-generated/yelloweye-spatial-index-cda-sim-200.rds")[[2]]
i_ye_noncda2 <- readRDS("data-generated/yelloweye-spatial-index-3CD-outside-cda-sim-200.rds")[[2]]
i_ye_5A2 <- readRDS("data-generated/yelloweye-spatial-index-5A-sim-200.rds")[[2]]
i_ye2 <- readRDS("data-generated/yelloweye-spatial-index-all-S-sim-200.rds")[[2]]
...
i_ye_2020 <- filter(.dat2, year == 2020) 

ye_catch_per_cell <- i_ye_2020 %>% group_by(Area) %>% summarise(
  catch = mean(.value/num_cells), 
  catch_lwr = quantile(.value/num_cells, 0.025), 
  catch_upr = quantile(.value/num_cells, 0.975))

ggplot(i_ye_2020) + geom_histogram(aes(.value/num_cells, fill = Area)) + 
  geom_vline(data = ye_catch_per_cell, aes(xintercept = catch), linetype = 2) +
  facet_grid(rows=vars(Area)) + xlab("Mean catch per 100 hooks") + ylab("Number of simulations") + 
  scale_fill_brewer(palette = "Set1") + theme(legend.position = "none")
ggsave("figs/yelloweye-mean-catch-sim.png", width = 3, height = 4, dpi = 400)
```
...
```{r echo=F}
ye_ratios <- i_ye_2020 %>% select(-num_cells) %>% pivot_wider(names_from = Area, values_from = .value) %>% 
  group_by(.iteration) %>% mutate(CDAto3CD = CDA/(CDA + `non-CDA 3CD`), CDAto3CD5A = CDA/(CDA + `non-CDA 3CD` + `5A`))
...
.dat <- left_join(.i_ye, .i_hal) %>% group_by(Area, year) %>% summarise(ye_per_hal = mean(yelloweye/halibut), 
...
g <- map_predictions(
  ...
  obs_data = filter(d_hal, year == 2020),
  fill_aes = ye_per_hal, fill_lab = "Predicted\nYelloweye\nper Halibut",
  size_aes = (catch_count / hook_count) * 100,
...
yemeanweights <- yesampledata %>% filter(!is.na(weight)) %>% group_by(year) %>% summarise(mean_weight = mean(weight/1000, na.rm = T), 
...
cda_ye_ids <- cda_ye_ids %>% select(fishing_event_id, year, catch_count) %>% distinct() # keep catch count for comparison with sample totals
...
cda_ye_samples <- inner_join(cda_ye_ids, yesampledata) %>% group_by(year) %>% mutate(cda_weights = n())
cda_ye_catch <- cda_ye_ids %>% group_by(year) %>% summarise(cda_catch = sum(catch_count))
cda_ye_sample_sum <- cda_ye_samples %>% select(year, cda_weights) %>% distinct()
cda_events <- cda_ye_ids %>% group_by(year) %>% summarise(cda_events = n())
cda_sampling <- left_join(cda_events, cda_ye_catch) %>% left_join(cda_ye_sample_sum) %>% mutate(catch_rate = round(cda_catch/cda_events, 1))
...
yeeventweights <- yesampledata %>% filter(!is.na(weight)) %>% 
  group_by(year, fishing_event_id) %>% 
  summarise(
...
```{r message=F, echo=F, warning=F}
yeeventweights1 <- yesampledata %>% filter(!is.na(weight)) %>% 
  group_by(year, fishing_event_id) %>% 
  summarise(
    mean_weight = mean(weight/1000, na.rm = T), 
