...
Trim prediction grid to cover full southern stock area
```{r}
full_s_grid_utm <- filter(s_grid_utm, latitude < 51.24999)
...
```{r eval = TRUE}
f <- "data-generated/halibut-predictions-S-outside-cda.rds"
if (!file.exists(f)) {
  ...
  saveRDS(i_hal_noncda, "data-generated/halibut-index-S-outside-cda.rds")
}
```
...
i_hal_noncda <- readRDS("data-generated/halibut-index-S-outside-cda.rds")

ggplot(i_hal_noncda, aes(year, est)) +
  geom_line() +
...
```{r message = F, echo= F}
ggplot(i_hal_noncda, aes(year, est / nrow(noncda_grid))) +
  geom_line() +
...
if (!file.exists("data-generated/yelloweye-spatial-predictions-S-outside-cda.rds")) {
  p_ye_noncda <- predict(m_ye, newdata = noncda_grid, return_tmb_object = TRUE)
  saveRDS(p_ye_noncda, "data-generated/yelloweye-spatial-predictions-S-outside-cda.rds")
  i_ye_noncda <- get_index(p_ye_noncda)
  saveRDS(i_ye_noncda, "data-generated/yelloweye-spatial-index-S-outside-cda.rds")
}
```
...
i_ye_noncda <- readRDS("data-generated/yelloweye-spatial-index-S-outside-cda.rds")
...
gh <- ggplot(data = focal_area) +
  geom_tile(
    data = filter(p_hal_cda$data, year == 2020),
    aes(longitude, latitude, fill = exp(est)), width = 0.06, height = 0.025
  ) +
  geom_tile(
    data = filter(p_hal_noncda$data, year == 2020),
