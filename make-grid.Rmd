...
depth2 <- raster::extract(x=rast_bat, y=new_grid_norca, na.rm = T, exact=T, sp = T, fun = mean)
mean_depth <- unlist(lapply(depth2, function(x) if (!is.null(x)) mean(x, na.rm=TRUE) else NA ))

all_depths <- bind_cols(hbll_grid_full_cells, mean_depth = mean_depth) %>% 
  mutate(cell_depth = -depth, depth_all = -mean_depth)

saveRDS(all_depths, "exrtracted_depths_norca.rds")

ggplot(all_depths, aes(cell_depth, depth_all, colour = area)) + geom_point(alpha=0.2)
...
ggplot(hbll_grid_keep) +
  geom_point(aes(X,Y, colour = -depth_all), alpha = 0.2) +
