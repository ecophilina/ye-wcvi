...
hbll_s_grid <- readRDS(here::here("grids/hbll_s_grid.rds")) # make sure it has the re-extracted cell coordinates
```
...
```{r}
grid_scale <- 2000 # currently using 2 x 2 km, but I think we could change this...
...
saveRDS(new_grid_norca, here::here("grids/hbll_shp_grid_norca_expanded.rds"))
...
saveRDS(new_grid_norca, here::here("grids/hbll_s_grid_norca_expanded.rds"))
}
```
...
```{r}
f <- "data/bath_res1_marmap2.rds"
if(!file.exists(f)){
# Load package
library(marmap)
new_grid_norca <- readRDS(here::here("grids/hbll_s_grid_norca_expanded.rds"))
# Download bathymetric data and save on disk
bat <- getNOAA.bathy(min(new_grid_norca$longitude)-2, max(new_grid_norca$longitude)+2,
  min(new_grid_norca$latitude)-2, max(new_grid_norca$latitude)+2, res = 1, keep = TRUE) #resolution of the grid, in minutes (default is 4 or > 4 miles)
saveRDS(bat, "data/bath_res1_marmap2.rds")
}
```
...
hbll_grid <- readRDS(here::here("grids/hbll_s_grid_norca_expanded.rds"))
...
new_grid_norca <- readRDS(here::here("grids/hbll_shp_grid_norca_expanded.rds"))
...
all_depths <- bind_cols(hbll_grid_full_cells, #mean_depth = mean_depth,
  wm_depth = mean_depth2, prop_water = water_area) %>% 
  mutate(cell_depth = -depth, #depth_uw = -mean_depth, 
    depth_wm = -wm_depth, depth = -wm_depth) %>% dplyr::select(#-mean_depth, 
      -wm_depth)

saveRDS(all_depths, here::here("grids/exrtracted_depths_norca_expanded.rds"))
...
f <- "data/highres-sandy-raster-expanded.grd" 
if (!file.exists(f)) {
...
saveRDS(all_covs, here::here("grids/norca_grid_w_substrate_expanded.rds"))
```
...
all_covs2 <- all_covs %>% mutate(area = area * prop_water) %>% filter(area > 0)
saveRDS(all_covs2, here::here("grids/norca_grid_w_substrate_expanded.rds"))
```
