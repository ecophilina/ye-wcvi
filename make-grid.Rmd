...
saveRDS(new_grid_norca, "hbll_shp_grid_norca.rds")
...
Download depth data for extent of grid
```{r}
# Load package
library(marmap)
...
# bat <- getNOAA.bathy(min(hbll_grid$longitude)-2, max(hbll_grid$longitude)+2,
#   min(hbll_grid$latitude)-2, max(hbll_grid$latitude)+2, res = 1, keep = TRUE) #resolution of the grid, in minutes (default is 4 or > 4 miles)
...
# make depth raster
bat <- readRDS( "data/bath_res1_marmap.rds")
...
new_grid_norca <- readRDS("hbll_shp_grid_norca.rds")
...
saveRDS(all_depths, "exrtracted_depths_norca.rds")
