...
CP <- as(raster::extent(
  min(hbll_s_grid$grid$x), max(hbll_s_grid$grid$x),
  min(hbll_s_grid$grid$y), max(hbll_s_grid$grid$y)
  ), "SpatialLines")
...
grid1 <- sf::st_make_grid(x = sa_poly,
  # offset = st_bbox(sa_poly)[c("xmin", "ymax")] + c(-1000, 1000),
...
# already in UTMs
new_grid_utm <- new_grid
new_grid_utm$geometry <- st_centroid(new_grid_utm) %>% st_geometry()

new_grid_ll <- st_transform(new_grid, crs = 4326)
new_grid_ll$geometry <- st_centroid(new_grid_ll) %>% st_geometry()

# new_grid3$geometry[1]

# # plot(st_geometry(new_grid))
# plot(st_geometry(new_grid_ll))
# plot(st_geometry(new_grid_utm))
# plot(st_geometry(new_grid_utm), cex = 0.2, add = T)
...
hbll_s_grid_ll <- as.data.frame(st_coordinates(new_grid_ll))

hbll_s_grid_utm <- as.data.frame(st_coordinates(new_grid_utm))

hbll_grid <- dplyr::select(hbll_s_grid_ll, X, Y) %>%
  rename(longitude = X, latitude = Y)

hbll_grid_utm <- dplyr::select(hbll_s_grid_utm, X, Y) %>%
  rename(x = X, y = Y) %>% mutate(X = x/ 100000, Y = y/ 100000)

hbll_grid <- bind_cols(hbll_grid, hbll_grid_utm)

hbll_s_grid <- list(grid = hbll_grid, cell_area = 2.0)
saveRDS(hbll_s_grid, here::here("grids/hbll_s_grid_filled.rds"))
