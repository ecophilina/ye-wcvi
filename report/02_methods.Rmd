...
For YE, we modelled biomass $B$ for point in space $s$ and time $t$ as
\begin{align}
  B_{s,t} &\sim \operatorname{Tweedie} \left(\mu_{s,t}, p, \phi \right), \: 1 < p < 2\ ,
\end{align}
...
\end{align}
where $\alpha_t$ represents a mean for each year, $\bm{X}_{s,t}$ represents a vector of predictors (survey ID, depth, depth squared, and a spline for proportion rocky and proportion muddy) and $\bm{\beta}$ represents a vector of corresponding parameters.
...
Our halibut model followed the same form as the YE model, but modelled biomass as a product of two component models:
\begin{align}
  B_{s,t} &\sim 
  \operatorname{Bernoulli} \left( \varphi_{s,t} \right) \cdot
    \operatorname{Gamma} \left( \mu_{s,t}, \frac{\mu_{s,t}}{\vartheta} \right),
\end{align}
where $\varphi$ represents the expected probability of catching any halibut in a given observation and $\mu$ represents the expected value conditional on a positive observation.
...
where the symbols have the same meaning as in the YE equations, but the subscript 1 and 2 denote the equivalent parameters from the first (Bernoulli) and second (Gamma) component models.
