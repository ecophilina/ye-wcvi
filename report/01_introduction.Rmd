...
```{r ye-ratio-by-ye-min-area, fig.cap="Variation in the mean biomass of \\textbf{YE to halibut} for cells with the \\textbf{lowest YE densities} up to the total area of all cells comprising the CDA. Mean YE densities of less than 1 g per 100 hooks have been truncated to this value and the ratios are then calculated for the combinations of cells with the lowest YE densities. Otherwise these ratios are derived in a similar way to the previous figure.", out.width="5.5in"}
include_graphics(here("figs/expected_YE_when_avoiding_YE_CI-hbll-est-rock-mud_allcda_filled.png"))
```
