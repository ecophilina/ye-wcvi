...
```{r halibut-preds-all, fig.cap="Model predicted landable Halibut biomass densities (colour) with survey sample densities (circle areas) overlaid. HBLL catch counts are converted to a landable biomass density by multiplying by the area swept per hook and the average landed biomass per fish caught. Trawl survey catch is converted an equvilant value by adjusting for lower catchability in trawl (model estimated) and the average proportion of biomass caught that belongs to landable individuals (set-specific proportions when available, or the global mean when not). The CDA is outlined in red. Note that the colour axis is square root transformed to improve visual interpretation.", out.width="\\textwidth"}
include_graphics(here("figs/filled-keepable-halibut-predictions-est-rock-mud.png"))
```

```{r ye-preds-all, fig.cap="Model predicted Yelloweye Rockfish biomass densities (colour) with survey sample densities (circle areas) overlaid. HBLL catch counts are converted to a biomass density by multiplying by the area swept per hook and the average biomass per fish caught in the survey overall years. Trawl survey catch is converted an equvilant value by adjusting for lower catchability in trawl (model estimated). Legend otherwise the same.", out.width="\\textwidth"}
