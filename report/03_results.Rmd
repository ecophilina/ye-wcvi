...
```{r effect-plots, fig.cap="Modelled quadratic effects of covariates: depth, proportion rocky substrate, and proportion muddy for each species' models. Because peak biomass densities occur in different years for each species, each line represents a different year's estimate with the most recent year having the darkest line. Shaded areas are each year's CI (confidence interval). These estimates are for HBLL survey catches of landable halibut and all YE. The effect of depth is for locations with avarage proportions of rocky (0.09) and muddy (0.13) substrate, and the substrate effects are at the depth at which YE densities peak (78 m) with the remainder of the substrate classified as either sandy or mixed.", out.width="\\textwidth"}
...
```{r ye-per-hal-preds-2020, fig.cap="Mapped ratios of Yelloweye Rockfish biomass to Halibut biomass in 2019--2020. Note that the colour axis is capped at the 99th percentile and is square root transformed to improve visual interpretation.", out.width="4.5in"}
...
(ref:fig-hal-per-ye-preds-2020) Mapped ratios of halibut biomass to Yelloweye Rockfish biomass in 2019--2020. Because YE often occur at much lower densities than Halibut, this calculation requires first truncating their biomass density to a minimum of 1 kg per kg/km^2^. Legend otherwise the same.
...
```{r indexes-avg, fig.cap="Average landable Halibut (A) and YE (B) biomass densities through time within the CDA, versus in the sampled range of Area 3CD5A outside the CDA. Lines are means and ribbons are 95\\% confidence intervals (CIs) from 500 draws from the joint-precision matrix.", out.width="5in"}
...
\clearpage
The ratio of YE to halibut within the CDA was on average lower than outside the CDA (Fig. \@ref(fig:ye-ratio-through-time)).
...
However, if selecting cells to avoid YE, the ratio of YE to landable halibut biomass expected within the CDA would increase sharply once more than 10% (or >250 kg/km^2^) of the CDA was sampled, while a lower YE ratio could be maintained while sampling from an equivelent area until the sampled area exceeded approximately `r signif(non_cda_exceeds_cda, digits = 2)` km^2^ of the non-CDA (Fig. \@ref(fig:ye-ratio-by-scenario)).
